...
class PhysicsSystem:
    ...
    def move_entity(self, entity_id: int, dt: float) -> bool:
        ...
        new_x = position.x + velocity.current_speed * math.cos(direction_rad) * dt
        new_y = position.y + velocity.current_speed * math.sin(direction_rad) * dt
    ...
    def set_velocity_towards(self, entity_id: int, target_x: float, target_y: float, speed: float):
        ...
        if distance > 0:
            ...
            # Set speed (respecting max speed limits)
            velocity.current_speed = min(speed, velocity.max_forward_speed)
